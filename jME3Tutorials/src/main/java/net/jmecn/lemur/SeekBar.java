...
public class SeekBar extends Panel {
	...
	/**
	 * Returns the slider range value for the specified location in the slider's
	 * local coordinate system. (For example, for world space location use
	 * slider.worldToLocal() first.)
	 */
	public double getValueForLocation(Vector3f loc) {

		Vector3f relative = loc.subtract(value.getLocalTranslation());

		// Components always grow down from their location
		// so we'll invert y
		relative.y *= -1;

		double projection = relative.x;
		if (projection < 0) {
			return model.getMinimum();
		}

		double rangeLength = value.getSize().x;
	...
	protected void resetStateView() {
		...
		double x = pos.x + visibleRange * model.getPercent();
	...
	private class ButtonDragger extends DefaultCursorListener {
		...
		public void cursorMoved(CursorMotionEvent event, Spatial target, Spatial capture) {
			...
			System.out.printf("%.2f -> %.2f\n", startPercent, startPercent+percent);
			event.setConsumed();
		}
	}
}
